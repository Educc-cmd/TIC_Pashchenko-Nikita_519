...
def main(anchorFrame, targetFrame, blockSize=16, outfile='output', saveOutput=True):
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
    ...
    if not isdir:
        os.mkdir(outfile)
    if saveOutput:
        cv2.imwrite(f"{outfile}/First frame.png", to_uint8(anchorFrame))
        cv2.imwrite(f"{outfile}/Second frame.png", to_uint8(targetFrame))
        cv2.imwrite(f"{outfile}/Difference between frame.png", to_uint8(diffFrameRGB))
        cv2.imwrite(f"{outfile}/Prediction frame.png", to_uint8(predictedFrameRGB))
        cv2.imwrite(f"{outfile}/Residual frame.png", to_uint8(residualFrameRGB))
        cv2.imwrite(f"{outfile}/Restore frame.png", to_uint8(restoreFrameRGB))
    ...
    plt.xticks([r + barWidth for r in range(len(P1))],['Біт/Піксель RGB', 'Біт/Піксель R', 'Біт/Піксель G', 'Біт/ПіксельB'])
    plt.legend()
